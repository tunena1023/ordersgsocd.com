...
/* Sin respaldo aqui a proposito (26/09/2026) -- son credenciales reales
   y este repo es publico. Las 5 GRAPH_* ya estan en Vercel (production,
   preview y development), igual que en Admin y Tech. */
...
const CLIENT_ADDRESSES_LIST = 'ClientAddresses';
const SERVICES_CATALOG_LIST = 'ServicesCatalog';
const SERVICE_TEMPLATES_LIST = 'ServiceTemplates';
/* Misma lista que ya usa Admingsocd.com (misma SharePoint) -- tiempos
   por servicio en minutos, para poder calcular el tiempo estimado
   aqui tambien sin duplicar la lista. */
const SERVICE_TIMES_LIST = 'ServiceTimes';
/* Version de un paquete por cliente (ClientID, PackageSKU, Items) -- ver lib/client-packages.js */
const CLIENT_PACKAGES_LIST = 'ClientPackages';
/* Mismas 2 listas de Admingsocd.com (misma SharePoint) -- Holidays es
   la lista maestra que Developer configura; ClientHolidays es la
   eleccion de cada cliente, capturada desde aqui (Profile). */
const HOLIDAYS_LIST = 'Holidays';
const CLIENT_HOLIDAYS_LIST = 'ClientHolidays';
const RECURRING_SERVICES_LIST = 'RecurringServices';
const RECURRING_LOG_LIST = 'RecurringLog';
const CLIENT_CONTACTS_LIST = 'ClientContacts';
/* "Assign by service" (Admin > Scheduling/Active) -- una fila por
   servicio-en-una-orden-en-modo-por-servicio. Misma lista real ya
   creada en SharePoint que usa Admingsocd.com, mismo nombre. */
const SERVICE_ASSIGNMENTS_LIST = 'ServiceAssignments';

/* Codigos de promo: canjean una orden de regalo (no un descuento en
   dolares -- este formulario no maneja precios). PROMO_CODES_LIST
   guarda el catalogo de codigos validos; REDEEMED_PROMO_CODES_LIST
   registra quien ya canjeo cual, para que nadie repita el mismo
   codigo 2 veces. Ambas listas hay que crearlas a mano en SharePoint
   (este entorno no llega a Graph para crearlas solo) con estas
   columnas:
     PromoCodes: Code (texto), Active (Si/No), MaxRedemptions
       (numero, opcional -- vacio = sin limite), TimesRedeemed (numero)
     RedeemedPromoCodes: ClientID (texto), Code (texto), RedeemedDate
       (texto/fecha) */
const PROMO_CODES_LIST = 'PromoCodes';
const REDEEMED_PROMO_CODES_LIST = 'RedeemedPromoCodes';
  ...
  try {
    ...
    return (data.value || []).map(x => ({
      ...
      /* URL de descarga temporal (pre-autenticada, ~1h) que Graph ya
         ...
         una imagen sin necesitar otro endpoint aparte. Mismo parche
         ya aplicado en Admingsocd.com y tech.gsocd.com -- a este
         repo nunca le habia llegado, causando que las fotos del
         cliente nunca cargaran (downloadUrl salia undefined). */
...
/* Nominatim (OpenStreetMap) -- gratis, sin cuenta, sin tarjeta. Nunca
   debe tronar quien la llame si falla o no encuentra nada; regresa
   null y ya. Copiada tal cual de Admingsocd.com/lib/graph.js --
   submit-order.js la necesita para geocodificar ordenes nuevas sin
   building ligado. */
...
module.exports = {
  ...
  CLIENT_HISTORY_LIST, CLIENT_ADDRESSES_LIST, CLIENT_CONTACTS_LIST, ORDERS_FOLDER, SERVICES_CATALOG_LIST, SERVICE_TEMPLATES_LIST, SERVICE_TIMES_LIST, CLIENT_PACKAGES_LIST,
  HOLIDAYS_LIST, CLIENT_HOLIDAYS_LIST, RECURRING_SERVICES_LIST, RECURRING_LOG_LIST,
  PROMO_CODES_LIST, REDEEMED_PROMO_CODES_LIST, SERVICE_ASSIGNMENTS_LIST,
