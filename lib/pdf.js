...
class PdfDoc {
  ...
  rightBlock(lines, opts = {}) {
    ...
    /* 'left': el bloque completo se ancla por su linea mas ancha (para
       que el borde derecho del bloque siga tocando el margen), pero
       cada linea EMPIEZA en ese mismo x -- a peticion del dueño, en
       vez de que cada linea termine alineada en el margen. */
    const leftX = align === 'left'
      ? this.width - this.margin - Math.max(...lines.map((ln, i) => widthOf(ln, size, i === 0 && opts.boldFirst)))
      : null;
    lines.forEach((ln, i) => {
