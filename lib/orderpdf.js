...
function renderServicesTable(doc, order, services, history) {
  ...
  /* BUG REAL encontrado con una orden real (GS-1032-1001, 20/09/2026,
     confirmado con el dueño): un servicio agregado durante la orden se
     quedaba con Result='Added' PARA SIEMPRE, incluso ya completada la
     orden -- 'Added' se checaba antes que Completed/Cancelled. Mismo
     fix aplicado en los 3 repos (cada uno tiene su propia copia de
     este archivo). Ahora el estatus final de la orden (o NotCompleted)
     siempre gana; 'Added' solo aplica mientras sigue en curso. */
  const rowFor = s => {
