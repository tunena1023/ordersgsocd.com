...
/* Busca el telefono del cliente en la lista Clients, por ClientID --
   a peticion del dueño (19/09/2026, con captura marcada: "el telefono
   del cliente! el que esta en el registro de esa orden! el
   contacto!"). El telefono NO vive en el registro de la orden (se
   confirmo revisando submit-order.js -- ninguna orden guarda Phone
   por su cuenta) ni en FieldEmployees (eso es telefono de STAFF/tech,
   cosa distinta) -- vive en Clients, campo 'Phone', un renglon por
   cliente (register-client.js). Si el cliente no tiene telefono
   guardado, o no se encuentra, regresa '' -- no truena, simplemente
   no se muestra nada de mas. NUNCA lanza hacia arriba (mismo patron
   que fetchLogoBuffer). */
async function findClientPhone(clientId) {
  if (!clientId) return '';
  try {
...
function renderClientSection(doc, order, includeContact) {
  ...
  /* A peticion del dueño (19/09/2026): mismo estilo que el bloque de
     ...
     en vez del formato anterior de una etiqueta por renglon. El
     telefono del cliente (campo 'Phone' en la lista Clients, buscado
     por ClientID en generateAndSaveOrderPdf/Completion/RequestPdf
     antes de llegar aqui -- NUNCA vivio en el registro de la orden)
     se pega junto al Contact, mismo patron "Nombre · Telefono" que ya
     se usa en otros lados de la app para el tecnico asignado. */
  ...
  const contact = clean(order.Contact);
...
async function generateAndSaveCompletionPdf(data) {
  ...
  try {
    ...
    const [logoBuffer, photos, clientPhone, techPhone] = await Promise.all([
      fetchLogoBuffer(),
      fetchOrderPhotoBuffers(order),
      findClientPhone(clientId),
      findTechPhoneByName(order.Supervisor)
    ]);
    const buffer = buildCompletionPdf(Object.assign({}, data, {
      order: Object.assign({}, order, { ClientPhone: clientPhone, TechnicianPhone: techPhone }),
...
async function generateAndSaveRequestPdf(data) {
  ...
  try {
    ...
    const [logoBuffer, clientPhone, techPhone, photos] = await Promise.all([
      fetchLogoBuffer(), findClientPhone(clientId), findTechPhoneByName(order.Supervisor),
      fetchOrderPhotoBuffers(order)
    ]);
