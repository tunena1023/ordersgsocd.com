...
const {
  ...
  CLIENTS_LIST, FIELD_EMPLOYEES_LIST, graphFetch, siteListPath
} = require('./graph');
...
async function generateAndSaveCompletionPdf(data) {
  ...
  try {
    ...
    const [logoBuffer, photos, clientInfo, techPhone] = await Promise.all([
      fetchLogoBuffer(),
...
async function generateAndSaveRequestPdf(data) {
  ...
  try {
    ...
    const [logoBuffer, clientInfo, techPhone, photos] = await Promise.all([
