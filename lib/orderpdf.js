...
function renderClientSection(doc, order, includeContact) {
  ...
  /* A peticion del dueño (19/09/2026, con captura marcada): "el
     Client ID debe ir arriba, y en donde esta el Client ID es donde
     debes poner el contacto y su telefono -- la idea es que se vea
     igual que el de nosotros [GS Solutions]... y no veo el email
     tampoco". Reacomodo: Client ID sale chiquito y gris, ARRIBA del
     nombre (no cuenta como uno de los renglones principales -- asi
     el bloque de abajo queda igual de largo que el de GS Solutions:
     nombre en negritas + 4 renglones). El correo (order.ClientEmail,
     campo 'Contact' en Clients -- mismo nombre confuso que ya causo
     el bug del nombre de la persona) se agrega al final, como ya
     tiene GS Solutions. */
  const email = clean(order.ClientEmail);
  if (clean(order.ClientID)) {
    doc.text('Client ID: ' + clean(order.ClientID), { size: 8.5, color: GRAY });
  }
