...
async function ownerOfOrder(g, orderId) {
  ...
  if (o) return { found: true, owner: fieldsOf(o).ClientID || '' };
  ...
  return d ? { found: true, owner: fieldsOf(d).ClientID || '' } : { found: false, owner: '' };
}

/* B12 (25/09/2026): antes, un orderId que no existia (o una orden sin
   ClientID) pasaba el candado. Ahora: orden sin dueño -> no pasa; orden
   que no existe -> no pasa, salvo en las funciones que reciben un id de
   borrador que todavia se esta creando. */
const ORDER_MAY_NOT_EXIST = new Set(['save-draft', 'submit-order', 'delete-draft']);
...
async function checkOwnership(g, slug, data, cid) {
  ...
  if (orderId) {
    const r = await ownerOfOrder(g, orderId);
    if (!r.found && !ORDER_MAY_NOT_EXIST.has(slug)) return 'This order was not found on your account.';
    if (r.found && !(r.owner && auth.sameClient(r.owner, cid))) return 'This order does not belong to your account.';
  }
